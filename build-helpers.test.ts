...
describe('generateCssPrefix', () => {
    describe('Basic functionality', () => {
        it('should handle regular package names', () => {
            ...
            expect(generateCssPrefix('react-components')).toBe('react-components');
            expect(generateCssPrefix('awesome_library')).toBe('awesome_library');
        });

        it('should remove @ symbol from the beginning only', () => {
            expect(generateCssPrefix('@company/package')).toBe('company-package');
        ...
        it('should replace forward slashes with hyphens', () => {
            expect(generateCssPrefix('@company/package-name')).toBe('company-package-name');
            expect(generateCssPrefix('namespace/sub/package')).toBe('namespace-sub-package');
            expect(generateCssPrefix('@org/utils/helpers')).toBe('org-utils-helpers');
        });

        it('should convert to lowercase', () => {
            expect(generateCssPrefix('@MyCompany/UI-Components')).toBe('mycompany-ui-components');
    ...
    describe('Scoped packages', () => {
        it('should handle typical scoped packages', () => {
            ...
            expect(generateCssPrefix('@storybook/react')).toBe('storybook-react');
        });

        it('should handle complex scoped package names', () => {
            expect(generateCssPrefix('@awesome-ui/react-components')).toBe('awesome-ui-react-components');
            expect(generateCssPrefix('@my-company/design-system-v2')).toBe('my-company-design-system-v2');
            expect(generateCssPrefix('@org/multi-word-package-name')).toBe('org-multi-word-package-name');
        });

        it('should handle scoped packages with numbers', () => {
            expect(generateCssPrefix('@company/ui-v2')).toBe('company-ui-v2');
            expect(generateCssPrefix('@babel/plugin-transform-es2015')).toBe('babel-plugin-transform-es2015');
    ...
    describe('Special characters and sanitization', () => {
        it('should remove invalid CSS characters', () => {
            ...
            expect(generateCssPrefix('my@pkg#with$symbols')).toBe('mypkgwithsymbols');
            expect(generateCssPrefix('@company/pkg.with.dots')).toBe('company-pkgwithdots');
        });

        it('should handle underscores (valid CSS character)', () => {
            expect(generateCssPrefix('my_package_name')).toBe('my_package_name');
            expect(generateCssPrefix('@company/ui_components')).toBe('company-ui_components');
        });
    ...
    describe('Edge cases', () => {
        it('should throw error for invalid inputs', () => {
            expect(() => generateCssPrefix('')).toThrow('Package name must be a non-empty string');
            expect(() => generateCssPrefix(null as any)).toThrow('Package name must be a non-empty string');
            expect(() => generateCssPrefix(undefined as any)).toThrow('Package name must be a non-empty string');
            expect(() => generateCssPrefix(123 as any)).toThrow('Package name must be a non-empty string');
        });
        ...
        it('should handle very long package names', () => {
            const longName = '@very-long-company-name/extremely-long-package-name-with-many-words';
            const result = generateCssPrefix(longName);
            expect(result).toBe('very-long-company-name-extremely-long-package-name-with-many-words');
    ...
    describe('Real-world package names', () => {
        it('should handle popular npm packages', () => {
            expect(generateCssPrefix('@babel/preset-env')).toBe('babel-preset-env');
            ...
            expect(generateCssPrefix('@storybook/addon-essentials')).toBe('storybook-addon-essentials');
        });

        it('should handle design system packages', () => {
            expect(generateCssPrefix('@material-ui/core')).toBe('material-ui-core');
            expect(generateCssPrefix('@chakra-ui/react')).toBe('chakra-ui-react');
            expect(generateCssPrefix('@ant-design/icons')).toBe('ant-design-icons');
            expect(generateCssPrefix('@headlessui/react')).toBe('headlessui-react');
        });

        it('should handle company-specific packages', () => {
            expect(generateCssPrefix('@company/design-tokens')).toBe('company-design-tokens');
            expect(generateCssPrefix('@acme-corp/ui-library')).toBe('acme-corp-ui-library');
            expect(generateCssPrefix('@my-org/shared-components')).toBe('my-org-shared-components');
        });
    });
...
describe('CSS class name generation integration', () => {
    ...
    it('should create predictable class names for components', () => {
        const generateClassName = (packageName: string, component: string, className: string) => {
            const prefix = generateCssPrefix(packageName);
            return `${prefix}__${component}__${className}`;
        };

        expect(generateClassName('@storybook/interactions', 'Button', 'primary'))
            .toBe('storybook-interactions__Button__primary');

        expect(generateClassName('@my-company/ui-lib', 'Card', 'elevated'))
            .toBe('my-company-ui-lib__Card__elevated');

        expect(generateClassName('design-tokens', 'Typography', 'heading'))
